...
- **4 cambios en `compu/index.html`:** (a) los **6** `.merch-item` cambiaron `href` de `https://elhueycoyote.myshopify.com/` (con target=_blank) → `href="#sec-merch" role="button"` (adiós Shopify, y sin abrir pestaña). (b) CSS `.merch-cartel` scoped a `#sec-merch` (centrado: `left:29%;top:19%;width:42%`, z-index 8, drop-shadow). (c) `<img class="merch-cartel">` dentro del `.ovl`. (d) JS init (mismo patrón que agotados: click en item → `preventDefault` + add clase + timer 3.5s; click en cartel → cierra).
- **Asset:** `Yamerito.png` optimizado a `movil/assets/compu/merch-cartel.png` (1200×788, RGB cuantizado a 32 colores conservando alpha → 521KB, a la par del cartel de agotados; bordes suaves intactos). Ref `?v=1`.
- **Verificado en Chrome real (Playwright, 1440px, abriendo el changarro):** click en item → `merch-cartel-on` presente, cartel llega a **opacity 1** tras el sello (~550ms), **centrado al 50%** y **dentro de la sección**, imagen carga 1200px. Auto-cierre a 3.5s confirmado (vuelve a estado oculto scale .6). `href` ya no apunta a Shopify (los 6). Sin errores de consola nuevos (solo favicon.ico 404 preexistente).
- **Móvil NO tocada.** Diff = `compu/index.html` + asset nuevo.
- **NOTA:** Ani reportó "me sigue mandando a Shopify" porque probó el sitio EN VIVO antes de que yo desplegara (el cambio estaba solo local). Se despliega en esta entrada.
...
- **Recuperación de alfa (`scratchpad/keyalpha.py`):** flood-fill de blanco (thr≥242) desde los bordes hacia adentro → sólo mata el blanco EXTERIOR conectado al borde; los blancos interiores (papel aluminio del póster central, blancos del logo) sobreviven. Luego `MinFilter(3)` (erosión 1px, se come el ringing JPEG) + `GaussianBlur(.6)`. Verificado: 0 halo en bordes al 400%; opacos 553k/686k/441k vs originales 553k/716k/441k.
- **Hallazgo:** el `conoces-poster2-full-v2.webp` viejo arrastraba un **fleco gris del muro** pegado (por eso su alpha-bbox iba a x=1484/y=28). El de Ani viene recortado limpio → bbox (720,64)-(1459,1143). Póster 1 y 3 calzan al píxel con los originales. El muro `bg-conoces-wall2.webp` YA estaba limpio y separado → no hizo falta pedirlo.
- **Assets nuevos:** `assets/compu/conoces-poster{1,2,3}-clean.webp` (RGBA 2032×1143, q92). Los viejos quedan sin uso.
- **Estructura del SVG de Ani:** viewBox `0 0 2031.9 1142.9` (= lienzo exacto). `<g id="Fondos">` = arte (incluye PARA/LATAM y EL VATO DE LAS ROLITAS RANDOM como texto → ya horneados en el PNG, NO se re-agregan). `<g id="Texto">` = overlay, 25 `<text>`.
- **Reconstrucción (`scratchpad/gen_overlay.py`):** parseo el `<style>` con selectores AGRUPADOS (`.stX, .stY {}` — un regex por clase suelta se pierde la mitad de las reglas), resuelvo cada clase, mapeo fuentes `Swiss721BT-Black→swiss-721-bt 900` / `-Bold→700` / `-BoldItalic→700 italic` (los 5 @font-face ya existen en compu), reescribo clases a prefijo `hx-*` para no chocar con las `st*` del sitio, y reparto los `<text>` por póster según la x del translate (<800 → p1, <1400 → p2, resto → p3).
- **⚠️ "El mero mero" EXCLUIDO a propósito** (clases st11/st4/st10/st1/st5/st8/st7): en compu ya es texto vivo en CSS (`.mero-mero`, se escribe solo con clip-path). Meterlo del SVG lo habría duplicado.
- **Cambio estructural clave:** en compu cada póster CAE por su cuenta (`conoces-drop-1/2/3`), así que el texto debe caer CON su póster. `<img class="poster-ov poster-ov-N">` → `<div class="poster-ov poster-ov-N"><img><svg class="poster-tx">…</svg></div>`. CSS: el div pierde `width:100%;height:auto` (eso era para el `<img>` reemplazado) y queda `inset:0`; `.poster-ov>img{display:block;width:100%;height:auto}` y `.poster-tx{position:absolute;top:0;left:0;width:100%;height:auto;overflow:visible}`. Verificado: svg e img alinean exacto (ambos top 89.75, h 810) y la animación arrastra ambos.
- **Barra gris p3:** el SVG traía `¿POR QUÉ SE HIZO ` cortado y desalineado. Puse el texto completo y lo centré con el mismo método del móvil: eje de la barra por regresión sobre `conoces-poster3-clean.webp` → **ángulo 4.635°, centro (1680.5, 819.09), largo 562.8, grosor 44.5**. Baseline = `819.09 + (34·0.719/2)·cos(4.635°)` = **831.3**, `text-anchor="middle"`, `rotate(4.635)`. Verificado en render: márgenes 61.5 izq / 58.7 der, mayúsculas centradas (lo que sobresale arriba es el acento de "QUÉ", correcto).
- **Gotcha de medición:** para mapear píxeles del render a coords SVG hay que tomar el `getBoundingClientRect()` del **svg**, no el de la `<section>` (difieren 40px). Y filtrar sólo por color gris agarra la estrellita y los bordes antialiaseados de la barra → acotar por |v| perpendicular.
- **Deploy:** commit `6ad9464` → GH Actions → elhueycoyote.com/preview/compu/
- **PENDIENTE:** nada de "¿Ya me conoces?" compu. Sigue: nada en móvil salvo si Ani cambia de opinión en títulos (ya confirmó que no).
